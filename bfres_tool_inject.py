...
def DDStoBFRES(ftex_pos, dds, bfres):
    ...
    if inb[:4] != b"FRES":
        ...
        lenfile = struct.unpack(">I", bytes(inb[0x0C:0x10]))[0]

        inb = bytearray(bytes(inb[:lenfile]))
        ...
        inb[ftex_pos+0xB0:ftex_pos+0xB4] = (lenfile - (ftex_pos+0xB0)).to_bytes(4, 'big')
        inb[ftex_pos+0xB4:ftex_pos+0xB8] = ((lenfile + dataSize) - (ftex_pos+0xB4)).to_bytes(4, 'big')

        if data_pos - (ftex_pos + 0xB0):
            inb[data_pos:data_pos+dataSize2] = b'\x00' * dataSize2
        if mip_pos - (ftex_pos + 0xB4):
            inb[mip_pos:mip_pos+mipSize2] = b'\x00' * mipSize2

        inb = bytearray(b''.join([bytes(inb), b''.join(result[:])]))

        inb[0x0C:0x10] = len(inb).to_bytes(4, 'big')
